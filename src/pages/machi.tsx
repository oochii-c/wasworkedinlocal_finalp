...
const ROLE_INFO: Record<RelationType, { label: string; emoji: string; desc: string; caution: string; base: number; color: string; badgePos: string }> = {
  gwiin: { label: "월묘", emoji: "🐰", desc: "챙겨야할 존재", caution: "받기만 하면 서운해질 수 있어요, 가끔은 나도 먼저 손을 내밀어보세요", base: 92, color: "#6FE0C0", badgePos: "top" },
  donghaeng: { label: "복덕이", emoji: "🐢", desc: "재물의 귀인", caution: "편하다고 소홀해지지 않게, 가끔은 마음을 표현해주세요", base: 68, color: "#F0C879", badgePos: "right" },
  oreunpal: { label: "호랑도사", emoji: "🐯", desc: "지혜의 귀인", caution: "내가 너무 많이 챙기면 지칠 수 있어요, 가끔은 기대도 괜찮아요", base: 82, color: "#C9A6F5", badgePos: "bottom-right" },
  horangi: { label: "악귀", emoji: "😭", desc: "운을 방해하는 존재", caution: "무리해서 맞추려 하지 마세요, 그냥 다른 리듬일 뿐이에요", base: 58, color: "#7FA8D9", badgePos: "bottom-left" },
  jageuk: { label: "구미호", emoji: "🦊", desc: "매력적인 귀인", caution: "부딪히는 게 나쁜 건 아니에요, 다만 감정 소모되지 않게 거리 조절을", base: 48, color: "#F08585", badgePos: "left" },
};
...
function MapDiagram({ participants, large, onClick }: MapDiagramProps) {
  ...
  return (
    ...
    >
      ...
      <p className="machi-map-caption">진주에 가까울수록, 빛이 밝을수록 케미가 좋은 사람</p>
    </div>
  );
}
  ...
  return (
    <div className="saju-page machi-page">
      ...
      <section className="saju-section">
        ...
        <SectionHeader title="상대방 태어난 날과 시간" />
