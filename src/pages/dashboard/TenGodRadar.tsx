...
import { SIPSHEN_LABELS, SIPSHEN_COLOR, strengthLabel, GRAPH_FULL_SHARE } from "./constants";
...
export default function TenGodRadar({ shiShenCount }: Props) {
  ...
  const innerPts = SIPSHEN_LABELS.map((lbl, i) => {
    ...
    // 전체합 대비 비율을 축 천장(GRAPH_FULL_SHARE)으로 정규화, 0.08은 최소 가시성 floor
    const fill = total > 0 ? Math.min(1, (count / total) / GRAPH_FULL_SHARE) : 0;
    return pentaPoint(CX, CY, R * Math.max(fill, 0.08), i);
  });
